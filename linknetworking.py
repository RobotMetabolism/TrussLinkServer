...
class LinkServer(threading.Thread):
    ...
    def __init__(self, host, port, log=False):
        ...
        # Allows the socket to bind to a previously used port
        self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    
        bound = False
        attempts = 0

        while not bound and attempts < 1000:
            try:
                safe_print("Trying to bind...")
                # binding will reaise OSError if port is already in use
                self.sock.bind((self.host, self.port))
                safe_print("Binded!")
                bound = True

            except OSError as e:
                safe_print(f"Port {self.port} in use. Trying next port {self.port + 1}...")    
                self.port +=1 # next port
                attempts += 1 # max attempt 1000
                time.sleep(1)
